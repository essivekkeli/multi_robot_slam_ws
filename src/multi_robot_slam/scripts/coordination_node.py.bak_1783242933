...
# ── Backoff parameters ────────────────────────────────────────────────────────
# If a goal aborts within this many sim-seconds of being sent, trigger cooldown.
FAST_ABORT_THRESHOLD_SEC = 5.0
# How long (sim-seconds) to withhold new goals after a fast abort.
COOLDOWN_SEC = 30.0
...
class RobotGoalState:
    ...
    def __init__(self, name):
        ...
        self._handle        = None
        self._goal_sent_sec = None   # sim-time (float, seconds) when goal was sent
        self._cooldown_until_sec = None  # sim-time after which robot is eligible again
        self._node_ref      = None   # set by CoordinationNode after construction
    ...
    def on_result(self, future):
        ...
        if status in (GoalStatus.STATUS_SUCCEEDED,
            ...
            # Check for fast abort — triggers cooldown
    ...
    def is_cooling_down(self, now_sec):
        """Returns True if robot is in cooldown and should not be assigned."""
...
class CoordinationNode(Node):
    def __init__(self, policy_path, algo, stochastic):
        ...
        # Give each RobotGoalState a back-reference so on_result can read sim-time
    ...
    def _goal_response_cb(self, future, robot_name, wx, wy):
        ...
        gs.goal_world = (wx, wy)
        gs._handle = handle
        # Record sim-time of goal acceptance for fast-abort detection
