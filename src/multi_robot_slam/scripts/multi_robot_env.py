...
class MultiRobotExplorationEnv:
    ...
    def step(self, actions):
        ...
        # Move robots toward current goals FIRST
        newly_assigned = []
        for i in range(N_ROBOTS):
            if self.goals[i] is not None:
                self._goal_age[i] += 1
                # Timeout: abandon unreachable goals after 15 steps
                if self._goal_age[i] > 15:
                    self.goals[i] = None
                    self._goal_age[i] = 0
        ...
        # Only assign new goals for robots that need one
        for i, a in enumerate(actions):
            if i in newly_assigned:
