#!/usr/bin/env python3
"""
Central Server â€” Stage 3a Week 3: Probabilistic (Log-Odds Bayesian) Map Fusion

Compared to Week 2 (ICP):
  - No small_gicp / no correction transforms / no alignment timer
  - merge_and_publish replaced with log_odds_merge() â€” Bayesian cell fusion
...
--------------
  ...
  Per-observation updates (per robot, per merge cycle):
    occupied  (100) â†’ += L_OCC  = +0.85   (P â‰ˆ 0.70)
    free      (  0) â†’ += L_FREE = -0.40   (P â‰ˆ 0.40)
    unknown   ( -1) â†’ no update

  Clamped to [-L_MAX, +L_MAX] = [-3.5, +3.5]

  Published occupancy:
    P > 0.65  â†’ 100  (occupied)
    P < 0.35  â†’   0  (free)
    otherwise â†’  -1  (unknown / conflicted)
"""
...
P_OCC_THRESH  = 0.65   # sigmoid(L) above this  â†’ publish 100
P_FREE_THRESH = 0.35   # sigmoid(L) below this  â†’ publish 0
# in between â†’ publish -1 (unknown / conflicted)


# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#  Shared helper functions
#  (identical across baseline, ICP, probabilistic
#   â€” do not modify, keeps Stage 3b comparison clean)
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
#  Probabilistic merge  (NEW â€” replaces the
...
def log_odds_merge(maps: dict, resolution: float):
    ...
    3. For each robot map, vectorised numpy scatter:
         occupied cells  â†’ += L_OCC
         free cells      â†’ += L_FREE
         unknown cells   â†’ no update (prior preserved)
    4. Clamp to [-L_MAX, +L_MAX].
    5. sigmoid(log_odds) â†’ probability â†’ threshold â†’ 0 / 100 / -1.

    Why the merge is better than occupied-wins
    ------------------------------------------
    Wall seen by 3 robots:  log-odds â‰ˆ 3Ã—0.85 = +2.55  â†’  P â‰ˆ 0.93  â†’  100
    Wall seen by 1 robot:   log-odds â‰ˆ +0.85           â†’  P â‰ˆ 0.70  â†’  100
    Free(r1) + Occ(r2):     log-odds â‰ˆ 0.85âˆ’0.40 = +0.45  â†’  P â‰ˆ 0.61
                            â†’ falls in [0.35, 0.65]  â†’  -1  (conflicted)
    ...
    # 5. Sigmoid â†’ probability â†’ occupancy
...
class CentralServer(Node):
    def __init__(self):
        ...
        # â”€â”€ ROS parameters â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Metrics directory (same structure as baseline and ICP) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        self.robot_maps        = {}
        self.global_map_latest = None      # for metrics â€” same as ICP
        self.lock = threading.Lock()

        # â”€â”€ Publishers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        # â”€â”€ Subscribers â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        for name in self.robot_namespaces:
    ...
    # â”€â”€ upstream pipeline (identical to baseline and ICP) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ probabilistic merge â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ metrics logging (identical structure to baseline and ICP) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
