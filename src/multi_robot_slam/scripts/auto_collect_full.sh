...
# Restarts Gazebo + GLIM + central server each cycle for stability
# Safe for 100+ run overnight collection
#
# Usage: bash auto_collect_full.sh 50
#
# Run in a dedicated terminal. Leave running overnight.
...
for i in $(seq 1 $N); do
    ...
    ros2 launch multi_robot_slam gz_multi_robot.launch.py \
      >> $LOG 2>&1 &
    GZ_PID=$!
    sleep 40   # wait for Gazebo + world_tf_broadcaster

    # ── Start IMU relays ─────────────────────────────────────────────
    echo "  [4/6] Starting IMU relays..."
    ros2 run multi_robot_slam imu_relay.py \
      --ros-args -p robot_name:=robot1 >> $LOG 2>&1 &
    ros2 run multi_robot_slam imu_relay.py \
      --ros-args -p robot_name:=robot2 >> $LOG 2>&1 &
    ros2 run multi_robot_slam imu_relay.py \
      --ros-args -p robot_name:=robot3 >> $LOG 2>&1 &
    sleep 3

    # ── Start GLIM x3 ────────────────────────────────────────────────
    echo "  [5/6] Starting GLIM..."
    ros2 run glim_ros glim_rosnode --ros-args \
      -p config_path:=$GLIM_CFG/robot1 \
      -p use_sim_time:=true \
      --remap /glim_ros/map:=/robot1/glim/map \
      --remap /glim_ros/odom:=/robot1/glim/odom \
      --remap /glim_ros/aligned_points:=/robot1/glim/aligned_points \
      >> $LOG 2>&1 &

    ros2 run glim_ros glim_rosnode --ros-args \
      -p config_path:=$GLIM_CFG/robot2 \
      -p use_sim_time:=true \
      --remap /glim_ros/map:=/robot2/glim/map \
      --remap /glim_ros/odom:=/robot2/glim/odom \
      --remap /glim_ros/aligned_points:=/robot2/glim/aligned_points \
      >> $LOG 2>&1 &

    ros2 run glim_ros glim_rosnode --ros-args \
      -p config_path:=$GLIM_CFG/robot3 \
      -p use_sim_time:=true \
      --remap /glim_ros/map:=/robot3/glim/map \
      --remap /glim_ros/odom:=/robot3/glim/odom \
      --remap /glim_ros/aligned_points:=/robot3/glim/aligned_points \
      >> $LOG 2>&1 &

    sleep 60   # wait for GLIM initialisation

    # ── Start central server + collector ─────────────────────────────
    python3 /ros2_ws/src/multi_robot_slam_py/multi_robot_slam_py/central_server.py \
      --ros-args -p method_label:=prob >> $LOG 2>&1 &
    CS_PID=$!

    python3 $SCRIPTS/collect_training_data.py >> $LOG 2>&1 &
    COL_PID=$!
    sleep 5

    # ── Drive robots ─────────────────────────────────────────────────
    echo "  [6/6] Driving robots..."
    timeout 900 python3 $SCRIPTS/drive_robots.py >> $LOG 2>&1
    drive_exit=$?

    if [ $drive_exit -eq 124 ]; then
        echo "  WARNING: drive timed out (15min) — saving anyway"
    fi

    # Wait for maps to stabilise then save
    sleep 8
